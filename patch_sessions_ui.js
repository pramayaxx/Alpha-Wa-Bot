...
const oldImports = `import { ShoppingCart, Bot, Puzzle, RefreshCcw, Check, Terminal, PlayCircle, Shield, Lock, LogOut, MessageSquare, Sparkles, Save, Server, Code, LayoutDashboard, Package, TrendingUp, Users, PauseCircle, Play, Image as ImageIcon, Mic } from 'lucide-react';`;
const newImports = `import { ShoppingCart, Bot, Puzzle, RefreshCcw, Check, Terminal, PlayCircle, Shield, Lock, LogOut, MessageSquare, Sparkles, Save, Server, Code, LayoutDashboard, Package, TrendingUp, Users, PauseCircle, Play, Image as ImageIcon, Mic, Smartphone, Plus } from 'lucide-react';`;
code = code.replace(oldImports, newImports);

const oldStates = `  const [sessionId, setSessionId] = useState('default');
  const [botState, setBotState] = useState({ status: 'offline', pairingCode: null, pairingError: null, qr: null });`;
const newStates = `  const [sessions, setSessions] = useState<string[]>(['default']);
  const [sessionId, setSessionId] = useState('default');
  const [botState, setBotState] = useState({ status: 'offline', pairingCode: null, pairingError: null, qr: null });`;
code = code.replace(oldStates, newStates);

const oldLoad = `      const res2 = await fetch('/api/shop/customers');`;
const newLoad = `      const resSessions = await fetch('/api/sessions');
      setSessions(await resSessions.json());
      const res2 = await fetch('/api/shop/customers');`;
code = code.replace(oldLoad, newLoad);

// Replace Connection Tab content
const oldConnTabStart = `{activeTab === 'connection' && (`;
const oldConnTabEnd = `            )}
            {activeTab === 'ai' && (`;

const startIndex = code.indexOf(oldConnTabStart);
const endIndex = code.indexOf(oldConnTabEnd);
...
if (startIndex > -1 && endIndex > -1) {
    ...
    code = code.substring(0, startIndex) + newConnTab + code.substring(endIndex + oldConnTabEnd.length);
    fs.writeFileSync('src/App.tsx', code);
    console.log("Patched UI successfully.");
} else {
    console.log("Could not find connection tab boundaries.");
}
